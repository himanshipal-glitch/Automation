...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if profit_df.empty:
        ...
        # Nothing NEW to review — but keep the already-saved decisions EDITABLE
        # right here (the top gate is gone once everything's reviewed), so a prior
        # choice can be changed anytime without hunting for the collapsed expander
        # below the summary. Ticking/unticking + Save re-computes the summary.
        if len(_cand):
            with st.expander(f"🧾 Reco Items — revise saved decisions "
                             f"({len(reco_ships)} of {len(_cand)} currently excluded)",
                             expanded=False):
                _render_reco_review(suffix="_top")
        ...
        # gate so saved decisions can always be revised. We no longer repeat the
        # full table here after the summary — it appeared twice and pushed the
        # email section out of view.
