...
with st.container(key="rkheader"):
    ...
    with _hdr_meta:
        ...
        with st.expander(f"{len(loaded)}/{len(status)} sheets · v3.2.3"):
            st.caption("build: **v3.2.3 — End Generator DN-invoice shipments added to the Reco review (user decides to keep/exclude)**")
            for sheet in loaded:
